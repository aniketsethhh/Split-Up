...
export const getAllContacts=query({
  ...
  .collect()).filter((e)=> e.paidByUserId !== currentUser._id &&
   e.splits.some((s)=>s.userId===currentUser._id));

   const personalExpanses =[...expensesYouPaid,...expensesNotPaidByYou];


   const contactIds = new Set();
   personalExpanses.forEach((exp) =>{
    if (exp.paidByUserId !== currentUser._id)
    contactIds.add(exp.paidByUserId);
  
    //Add each user in the split that isnot the current user
   ...
   .filter((g)=> g.members
        ...
        //step8 sort result alphabetecaly by name
